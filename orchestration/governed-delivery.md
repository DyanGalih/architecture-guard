---
description: Run the recommended resumable plan-to-tasks workflow with memory context, security review, architecture gates, and adapter analysis.
---
...
3. Detect `flash-mem` as an MCP service. Do not look for it in `{adapter_path:extensions}`.
4. Detect Security Review from the adapter when it declares a supported extensions artifact; otherwise use host capability detection. Accept `security-review` as the canonical extension id and `spec-kit-security-review` as a compatibility alias.
...
If the plan is `missing` or `stale`, execute the adapter-registered governed-plan capability.
...
If tasks are `missing`, `stale`, or `review-required`, execute the adapter-registered governed-tasks capability with the accepted plan and cached context.
