...
exports.COMMANDS = exports.AGENT_CONFIGS = void 0;
exports.runInstallCommand = runInstallCommand;
exports.installCommand = installCommand;
exports.appendAgentsMd = appendAgentsMd;
exports.installToml = installToml;
exports.installYaml = installYaml;
exports.validateRuntimeResources = validateRuntimeResources;
const fs = require('fs');
...
exports.AGENT_CONFIGS = AGENT_CONFIGS;
const COMMANDS = [
    'init',
    'init-brownfield',
    'governed-discover',
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        switch (a) {
            ...
            case '--command':
                ...
                break;
            case '--':
                opts.values = argv.slice(i + 1);
                i = argv.length;
                break;
            default:
                if (a.startsWith('--agent='))
                    opts.agents = a.slice('--agent='.length);
                else if (a.startsWith('--framework='))
                    opts.framework = a.slice('--framework='.length);
                else if (a.startsWith('--commands='))
                    opts.commands = a.slice('--commands='.length);
                else if (a.startsWith('--overwrite='))
                    opts.overwrite = a.slice('--overwrite='.length);
...
async function runInit(targetDir, opts) {
    ...
    if (selectedAgents.includes('antigravity')) {
        if (!userPath) {
            ...
            if (scopeChoice.startsWith('Global'))
                agyScope = 'global';
            else if (scopeChoice.startsWith('Shared'))
                agyScope = 'shared';
            else
                agyScope = 'workspace';
        }
...
async function runInstallCommand(target, opts) {
    opts.target = target;
