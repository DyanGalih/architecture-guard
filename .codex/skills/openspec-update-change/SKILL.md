...
1. **Select the change**
   ...
   - If ambiguous, run `openspec list --toon` to get available changes sorted by most recently modified, and ask the user to select one
...
- `$openspec-continue-change` and `$openspec-new-change` may not be installed (core profile). When suggesting one that is unavailable, point to the CLI instead: `openspec status --change "<name>" --toon` shows the next artifact and `openspec instructions <artifact-id> --change "<name>" --toon` explains how to create it.
