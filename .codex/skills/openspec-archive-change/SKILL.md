...
1. **Select the change**
   ...
   **Load current archive inputs before the existing archive checks:**
   ...
   ```bash
   openspec instructions archive --change "<name>" --toon
   ```
...
2. **Check artifact completion status**
   ...
   Run `openspec status --change "<name>" --toon` to check artifact completion.
...
4. **Assess delta spec sync state**
   ...
   **Prompt options:**
   ...
   `openspec instructions specs --change "<name>" --toon` once with the same
...
- Use artifact graph (openspec status --toon) for completion checking
