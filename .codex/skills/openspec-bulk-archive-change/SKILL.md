...
1. **Get active changes**
   ...
   Run `openspec list --toon` to get all active changes.
...
2. **Prompt for change selection**
   ...
   **Load current archive inputs once for the selected root before batch validation:**
   ...
   `openspec instructions archive --change "<selected-change>" --toon` with the
...
3. **Batch validation - gather status for all selected changes**
   ...
   a. **Artifact status** - Run `openspec status --change "<name>" --toon`
...
7. **Confirm batch operation**
   ...
   `openspec instructions specs --change "<name>" --toon` exactly once with the
